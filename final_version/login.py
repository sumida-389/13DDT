...
def login_screen(self):
    ...
    self.left_panel(head="Focalize", subtext="Knowledge at your fingertips.")
    ...
    tk.Label(
        sub_text, text="No account? ", bg="white", fg=LIGHT_GREY, font=("Helvetica", 11)
    ).pack(side="left")
    tk.Label(
        sub_text,
        text="Register here",
    ...
    status = tk.Label(form, text="", fg=DARK_RED, font=("Helvetica", 11))
...
def register_screen(self):
    ...
    clear_screen(self.root)
    self.left_panel(head="Create Account", subtext=" ")
    ...
    sub.winfo_children()[1].bind(
        "<Button-1>", lambda e: self.login_screen()
    ...
    # Status message
    status = tk.Label(form, text="", bg="white", fg=DARK_RED, font=("Helvetica", 11))
    status.pack(anchor="w", pady=(0, 8))
    ...
    def attempt_register():
        ...
        if username in ("Username", ""):
            ...
            return
        if len(username) > 12:
            status.config(text="Username must be 12 characters or less.")
            return
