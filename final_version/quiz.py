...
def quiz_screen(self):
    ...
    header.pack(fill="x")
    tk.Label(
        header, text="Quizzes", font=("Helvetica", 20, "bold"), bg=DARK_RED, fg="white"
    ...
    def refresh_quiz_list():
        ...
        for widgets in list_frame.winfo_children():
            widgets.destroy()
        cursor.execute(
            "SELECT id, title FROM quizzes WHERE user_id=?", (self.current_user_id,)
        )
        quizzes = cursor.fetchall()
        if not quizzes:
            tk.Label(
                list_frame, text="No quizzes yet!", bg="white", fg=LIGHT_GREY
            ).pack(pady=20)
            return
        ...
        for qid, qtitle in quizzes:
            ...
            tk.Label(row, text=qtitle, bg=GREY_BG, font=("Helvetica", 12)).pack(
            ...
            def delete_quiz(qid):
                if messagebox.askyesno(
                    "Delete", "Delete this quiz and all its questions?"
                ):
            ...
            tk.Button(
                row, text="Delete", fg=DARK_RED, command=lambda i=qid: delete_quiz(i)
...
def quiz_edit_screen(self, quiz_id, quiz_title):
    ...
    for i, label in enumerate(
        ...
        tk.Label(form, text=label + ":", bg=GREY_BG, width=12, anchor="w").grid(
    ...
    tk.Label(form, text="Correct Answer:", bg=GREY_BG, width=12, anchor="w").grid(
        row=5, column=0, sticky="w"
    )
    correct_var = tk.StringVar(value="A")
    # Create a dropdown menu for selecting the correct answer
    tk.OptionMenu(form, correct_var, "A", "B", "C", "D").grid(
    ...
    def load_questions():
        ...
        if not rows:
            tk.Label(
                q_frame, text="No questions yet!", bg="white", fg=LIGHT_GREY
            ).pack()
            return
        ...
        for qid, qtxt, qans in rows:
            ...
            tk.Button(
                ...
                text="Delete",
                fg=DARK_RED,
                command=lambda i=qid: delete_question(i),
                bg=DARK_RED,
...
def take_quiz(self, quiz_id, quiz_title):
    ...
    def show_question():
        ...
        def submit():
            ...
            if ans == correct:
                ...
                feedback.config(
                    text=f"✘  Wrong. The correct answer is {correct}", fg=DARK_RED
                )
            # Disable the submit button after submission to prevent multiple submissions
            submit_btn.config(state="disabled")
            win.after(
                1200, lambda: [state.update({"idx": state["idx"] + 1}), show_question()]
            )
    ...
    def show_result():
        ...
        tk.Label(
            win, text=f"{state['score']} / {state['total']}", fg="white", bg=DARK_RED
        ).pack()
        tk.Button(
            win, text="Close", bg=DARK_RED, fg="black", command=win.destroy
        ).pack()
