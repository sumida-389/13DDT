...
def calendar_screen(self):
    ...
    header.pack(fill="x")
    tk.Label(
        header, text="Calendar", font=("Helvetica", 20, "bold"), fg="white", bg=DARK_RED
    ...
    month_lbl = tk.Label(
        nav, text="", bg="white", fg=DARK_RED, font=("Helvetica", 26, "bold"), width=20
    )
    ...
    def open_add_event_popup(event=None):
        ...
        tk.Label(form, text="Title", bg=GREY_BG, font=("Helvetica", 10)).grid(
        ...
        tk.Label(
            form, text="Date (YYYY-MM-DD)", bg=GREY_BG, font=("Helvetica", 10)
        ...
        tk.Label(form, text="Type", bg=GREY_BG, font=("Helvetica", 10)).grid(
            row=2, column=0, sticky="w", padx=4, pady=4
        )
        type_var = tk.StringVar(value="exam")
        tk.OptionMenu(form, type_var, "exam", "assignment", "study", "other").grid(
        ...
        tk.Button(
            form, text="Add Event", command=add_event, font=("Helvetica", 10)
    ...
    def actual_calendar():
        ...
        for d in range(1, days_in + 1):
            ...
            day_lbl = tk.Label(
                ...
                text=str(d),
                bg=bg,
                fg=DARK_RED if d == today else "black",
            ...
            for ev_title, ev_type in evts[:max_shown]:
                ...
                tk.Label(
                    ev_row, text="●", bg=bg, fg=swatch_color, font=("Helvetica", 8)
                ).pack(side="left")
                tk.Label(
                    ev_row,
                    text=ev_title,
            ...
            if len(evts) > max_shown:
                ...
                tk.Label(
                    cell,
                    text=f"+{len(evts) - max_shown} more",
                    bg=bg,
                    fg=LIGHT_GREY,
                    font=("Helvetica", 8),
    ...
    for event_type, colour in TYPE_COLORS.items():
        ...
        tk.Label(item, text="●", fg=colour, bg="white", font=("Helvetica", 15)).pack(
            side="left"
        )
